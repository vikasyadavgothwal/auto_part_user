                ...
                <div className="overflow-x-auto rounded-sm border border-border">
                  <table className="w-full min-w-[760px] text-sm">
                    <thead className="bg-brand-surface text-brand-muted">
                      <tr><th rowSpan={2} className="p-3 text-left">VIN</th><th rowSpan={2} className="p-3 text-left">Part</th><th rowSpan={2} className="p-3 text-left">Number</th><th rowSpan={2} className="p-3 text-left">Qty</th><th rowSpan={2} className="p-3 text-left">Target</th>{selected.bids.map((bid, index) => <th key={bid.id} colSpan={4} className="border-l border-border p-3 text-left"><div className="flex min-w-[410px] items-center justify-between gap-3"><div><div className="mb-1 flex items-center gap-2"><p className="font-semibold text-foreground">Vendor {index + 1}</p>{bid.featuredSupplier ? <Badge className="rounded-full bg-primary px-2 py-0.5 text-[11px] text-primary-foreground">Featured supplier</Badge> : null}</div><p className="text-xs font-normal">{supplierName(bid)} · {money(bid.totalAmount)} · up to {bid.deliveryDays} days</p></div>{selected.status === "open" && bid.status === "submitted" ? <Button size="sm" disabled={Boolean(accepting)} onClick={() => setConfirmBidId(bid.id)}>Accept Quote</Button> : <span className="text-xs capitalize">{bid.status}</span>}</div></th>)}</tr>
